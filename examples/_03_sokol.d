...
auto state = state_t(color_t(90.0f, 95.0f, 100.0f));

struct color_t { float r = 0, g = 0, b = 0; }
struct state_t { color_t bg; mu_Context mu_ctx; }
...
void demo_window(mu_Context* ctx) {
    ...
    if (mu_begin_window(ctx, "Demo Window", mu_rect(40, 40, 300, 450))) {
        ...
        if (mu_header(ctx, "Window Info")) {
            ...
            static char[69] buf = '\0';
            mu_layout_row(ctx, 0, 54, -1);
            mu_label(ctx,"Position:");
            sprintf(buf.ptr, "%d, %d", win2.rect.x, win2.rect.y); mu_label_legacy(ctx, buf.ptr);
            mu_label(ctx, "Size:");
            sprintf(buf.ptr, "%d, %d", win2.rect.w, win2.rect.h); mu_label_legacy(ctx, buf.ptr);
        }

        /* labels + buttons */
        if (mu_header_ex(ctx, "Test Buttons", MU_OPT_EXPANDED)) {
            mu_layout_row(ctx, 0, 86, -110, -1);
            mu_label(ctx, "Test buttons 1:");
            if (mu_button(ctx, "Button 1")) { puts("Pressed button 1"); }
            if (mu_button(ctx, "Button 2")) { puts("Pressed button 2"); }
            mu_label(ctx, "Test buttons 2:");
            if (mu_button(ctx, "Button 3")) { puts("Pressed button 3"); }
            if (mu_button(ctx, "Popup")) { mu_open_popup(ctx, "Test Popup"); }
            if (mu_begin_popup(ctx, "Test Popup")) {
                if (mu_button(ctx, "Hello")) { puts("Pressed hello"); };
                if (mu_button(ctx, "World")) { puts("Pressed world"); };
                mu_end_popup(ctx);
            }
        }
        ...
        if (mu_header_ex(ctx, "Tree and Text", MU_OPT_EXPANDED)) {
            ...
            if (mu_begin_treenode(ctx, "Test 2")) {
                mu_layout_row(ctx, 0, 54, 54);
                if (mu_button(ctx, "Button 3")) { puts("Pressed button 3"); }
                if (mu_button(ctx, "Button 4")) { puts("Pressed button 4"); }
                if (mu_button(ctx, "Button 5")) { puts("Pressed button 5"); }
                if (mu_button(ctx, "Button 6")) { puts("Pressed button 6"); }
                mu_end_treenode(ctx);
            }
        ...
        if (mu_header_ex(ctx, "Background Color", MU_OPT_EXPANDED)) {
            mu_layout_row(ctx, 74, -78, -1);
            /* sliders */
            mu_layout_begin_column(ctx);
            mu_layout_row(ctx, 0, 46, -1);
            ...
            mu_draw_rect(ctx, r, mu_color(cast(ubyte) state.bg.r, cast(ubyte) state.bg.g, cast(ubyte) state.bg.b, 255));
            static char[420] buf = '\0';
            sprintf(buf.ptr, "#%02X%02X%02X", cast(int) state.bg.r, cast(int) state.bg.g, cast(int) state.bg.b);
...
void r_init() {
    ...
    uint rgba8_size = ATLAS_WIDTH * ATLAS_HEIGHT * 4;
    uint* rgba8_pixels = cast(uint*) malloc(rgba8_size);
    for (int y = 0; y < ATLAS_HEIGHT; y++) {
        for (int x = 0; x < ATLAS_WIDTH; x++) {
            int index = y * ATLAS_WIDTH + x;
            rgba8_pixels[index] = 0x00FFFFFF | (cast(uint) atlas_texture[index] << 24);
        }
    }
    sg.ImageDesc image_desc;
    image_desc.width = ATLAS_WIDTH;
    image_desc.height = ATLAS_HEIGHT;
    image_desc.data.subimage[0][0].ptr = rgba8_pixels;
    image_desc.data.subimage[0][0].size = rgba8_size;
    atlas_img = sg.makeImage(image_desc);
    ...
    sampler_desc.min_filter = sg.Filter.Nearest;
    sampler_desc.mag_filter = sg.Filter.Nearest;
    atlas_smp = sg.makeSampler(sampler_desc);

    sg.PipelineDesc pipeline_desc;
    pipeline_desc.colors[0].blend.enabled = true;
    pipeline_desc.colors[0].blend.src_factor_rgb = sg.BlendFactor.Src_alpha;
    pipeline_desc.colors[0].blend.dst_factor_rgb = sg.BlendFactor.One_minus_src_alpha;
    pip = sgl.makePipeline(pipeline_desc);
...
void r_begin(int disp_width, int disp_height) {
    ...
    sgl.pushMatrix();
    sgl.ortho(0.0f, cast(float) disp_width, cast(float) disp_height, 0.0f, -1.0f, +1.0f);
    sgl.beginQuads();
}
...
void r_push_quad(mu_Rect dst, mu_Rect src, mu_Color color) {
    float u0 = cast(float) src.x / cast(float) ATLAS_WIDTH;
    float v0 = cast(float) src.y / cast(float) ATLAS_HEIGHT;
    float u1 = cast(float) (src.x + src.w) / cast(float) ATLAS_WIDTH;
    float v1 = cast(float) (src.y + src.h) / cast(float) ATLAS_HEIGHT;

    float x0 = cast(float) dst.x;
    float y0 = cast(float) dst.y;
    float x1 = cast(float) (dst.x + dst.w);
    float y1 = cast(float) (dst.y + dst.h);
...
void r_draw_text(const(char)* text, mu_Vec2 pos, mu_Color color) {
    ...
    for (const(char)* p = text; *p; p++) {
        mu_Rect src = atlas[ATLAS_FONT + cast(ubyte) *p];
        dst.w = src.w;
        dst.h = src.h;
...
@trusted
int r_get_text_width(const(char)[] text) {
    int res = 0;
    auto len = text.length;
    for (const(char)* p = text.ptr; *p && len--; p++) {
        res += atlas[ATLAS_FONT + cast(ubyte)*p].w;
    }
    return res;
}
...
const(char)[347] key_map = [
    sapp.Keycode.Left_shift    : MU_KEY_SHIFT,
    sapp.Keycode.Right_shift   : MU_KEY_SHIFT,
    sapp.Keycode.Left_control  : MU_KEY_CTRL,
    sapp.Keycode.Right_control : MU_KEY_CTRL,
    sapp.Keycode.Left_alt      : MU_KEY_ALT,
    sapp.Keycode.Right_alt     : MU_KEY_ALT,
    sapp.Keycode.Enter         : MU_KEY_RETURN,
    sapp.Keycode.Backspace     : MU_KEY_BACKSPACE,
];
